...
void printTable(){
	...
	while( symTabPtr != NULL ){
      ...
      if (symTabPtr->first != NULL){
		...
		while(paramPtr != NULL ){
             printf(" paramType %d ", paramPtr->type);
...
void changeDesc(char* name, Type returntype){
	...
	if(returntype == PARAM){
	/*	if(paramOffsetCounter == 1){
			symTabPtr->offset = 8;
		}else{
			symTabPtr->offset = 4 * paramOffsetCounter;
		}
...
int tempVariablesSize(){
	...
	while(strcmp(ptr->id, "$") != 0){//iterate over the table and get the size of the temps
		if(ptr->description == PARAM){
			symTabNode * funcPointer = state->next->table;
			if(funcPointer->size < decrement){
				ptr->offset = 8;
			}else{
				ptr->offset = ((funcPointer->size - decrement) + 1 ) * 4;
